            ...
            using (var db = new AppDbContext())
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (Image != null && Image.ContentLength > 0)
                    {
                        ...
                        // 保存圖片的存儲相對路徑到資料庫
                        product.Image = Path.Combine("Products", fileName);
                    }
        ...
        public ActionResult Update(ProductVm model, HttpPostedFileBase Image)
        {
            using (var db = new AppDbContext())
            {
                if (ModelState.IsValid)
                {
                    ...
                    // 處理圖片邏輯
                    if (model.DeleteImage)  // 如果用戶選擇刪除圖片
                    {
                        // 刪除當前圖片（如果有需要，也可以刪除伺服器上的圖片文件）
                        product.Image = null;
                    }
                    else if (Image != null && Image.ContentLength > 0)  // 如果有新圖片上傳
                    {
                        // 獲取文件名
                        var fileName = Path.GetFileName(Image.FileName);
                    ...
                    return Json(new { success = true, message = "商品與加選資訊已成功更新" });
                }
